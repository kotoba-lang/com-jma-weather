(ns jma_weather.main
  "Kotodama WASM entrypoint for the Jma_weather clean-room actor (L4) — Clojure port.
  ...
  facts (`jma_weather.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "jma_weather")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"jma_weather.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "jma_weather-compat" :tier tier :entities entities} 200])
...
(defn start! [] :jma_weather-compat/ready)
